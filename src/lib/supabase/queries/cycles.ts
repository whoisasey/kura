...
export const getLast6Cycles = async (
  supabase: SupabaseClient,
  userId: string
): Promise<Cycle[]> => {
  const { data } = await supabase
...
export const getAvgCycleLength = async (
  supabase: SupabaseClient,
  userId: string
): Promise<number> => {
  const { data } = await supabase
...
export const insertCycle = async (
  supabase: SupabaseClient,
  data: { user_id: string; period_start: string; period_end?: string; phase?: string }
): Promise<Cycle> => {
...
export const updateCycleEnd = async (
  supabase: SupabaseClient,
  cycleId: string,
  periodEnd: string
): Promise<void> => {
  ...
  const cycleLength = existing
    ? Math.round(
        (new Date(periodEnd).getTime() - new Date(existing.period_start).getTime()) /
          (1000 * 60 * 60 * 24)
      ) + 1
    : null
