...
function buildSymptomHistory(journalWithSymptoms: JournalWithSymptoms[], cycles: Cycle[]): string {
  ...
  for (const entry of journalWithSymptoms) {
    ...
    const day =
      Math.floor(
        (new Date(entry.entry_date).getTime() - new Date(cycle.period_start).getTime()) / (1000 * 60 * 60 * 24)
      ) + 1;
  ...
  return Array.from(byCycle.entries())
    .map(([start, days]) => {
      const dayLines = days
        .sort((a, b) => a.day - b.day)
        .map((d) => `day ${d.day}: ${d.symptoms.map((s) => `${s.symptom} (sev ${s.severity})`).join(", ")}`)
        .join("; ");
